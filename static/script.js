document.addEventListener('DOMContentLoaded', function() {
    ...
    function fetchPage(page) {
        ...
        fetch(`/search_dynamic?query=${encodeURIComponent(query)}&cost=${costValues.join(',')}&power=${powerValues.join(',')}&page=${page}`) // Added encodeURIComponent here
            ...
            .then(data => {
                ...
                data.cards.forEach(card => {
                    resultsHtml += `
                        <div class="card">
                            <a href="/card/${card.cid}" class="card-link"> <h2>${card.name}</h2>
